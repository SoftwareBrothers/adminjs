import { Box, combineStyles, cssClass, selectStyles } from '@adminjs/design-system'
...
const BrandingSelector: FC<BrandingSelector> = (props) => {
  ...
  const [, storeTheme] = useLocalStorage<BrandingSelector | null>('adminjs-branding', null)
  const { translateMessage } = useTranslation()

  const handleChange = useCallback(async (event: SelectableBranding) => {
    setSelected(event)

    const branding: BrandingOptions = typeof event.value === 'function' ? await event.value() : event.value
  ...
  return (
    <SelectContainer padding="xl">
      <Select
        placeholder={translateMessage('brandingSelectorSearch')}
        value={selected}
