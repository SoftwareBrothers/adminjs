import React, {
  MouseEvent, useCallback, useMemo, memo, useRef,
} from 'react';
import {
  Button, Section, FormGroup, FormMessage, Icon, Box,
...
const encodeId = (
  value: string,
  path: string,
) => window.btoa(unescape(encodeURIComponent(`${value}${VALUE_PATH_SEPARATOR}${path}`)));
...
const ItemRenderer: React.FC<EditProps & ItemRendererProps> = memo((props) => {
  ...
  const {
    ItemComponent, property, onDelete, record, findItem, moveItem, isDraggable,
  } = props;
...
const InputsInSection: React.FC<EditProps> = memo((props) => {
  const {
    property, record, resource, onChange,
  } = props;
